...
// SQL engines.
//
// The package core is intentionally driver-agnostic.
// Database-specific constructors live in separate files.
...
//
// Design rationale:
//   - writes are forced through a single lane to keep order deterministic;
//   - reads are spread across multiple DB handles for parallelism;
//   - synchronization is done only with channels and select.
type Queue struct {
	in     chan Task
	readQ  chan Task
	writeQ chan Task
...
//
// The core pipeline is independent from any specific SQL engine.
func startQueue(writerDB *sql.DB, readerDBs []*sql.DB) *Queue {
	...
	// This keeps workers simple and free from mode branching.
	go func() {
		defer close(q.readQ)
	...
	// Readers execute read tasks in parallel on separate DB handles.
	for _, db := range readerDBs {
		db := db
...
func normalizeValue(rawValue any) any {
